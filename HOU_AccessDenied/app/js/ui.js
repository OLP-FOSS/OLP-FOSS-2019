...
function init() {
    ...
    setInterval(function () {
        var date = new Date();
        $('#timeInfo').html(date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds());
    }, 1000);
...
window._countinfo = 0;
window._counting_timer = -1;

var countingfunc = function () {
    if (window._countinfo <= 0) {
        clearInterval(window._counting_timer);
    }
    if (window._countinfo >= 60) {
        $('#countingInfo').html(Math.floor(window._countinfo / 60) + ':' + (window._countinfo % 60));
    } else {
        $('#countingInfo').html('00:' + window._countinfo);
    }
    window._countinfo--;
/*
    window._countinfo--;
...
var resetCountingInfo = function (secs) {
    if (window._counting_timer !== -1) {
        clearInterval(window._counting_timer);
    }
    window._countinfo = secs;
    if (window._countinfo != 0) {
        window._counting_timer = setInterval(countingfunc, 1000);
    }
}
